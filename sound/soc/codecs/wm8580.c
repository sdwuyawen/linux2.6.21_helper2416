 ...
 * Set PCM DAI bit size and sample rate.
 */
static int wm8580_saif_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai_link *dai = rtd->dai;
...
//	u16 saifb1;// = wm8580_read(codec, WM8580_SAIF1 + dai->codec_dai->id);
	wm8580_write(codec, 0x32 + dai->codec_dai->id, 0x3c);
	wm8580_write(codec, 0x33 + dai->codec_dai->id, 0x3f);
	wm8580_write(codec, 0x8 + dai->codec_dai->id, 0x10);

	wm8580_write(codec, 0x1d + dai->codec_dai->id, 0x140);
	wm8580_write(codec, 0xb + dai->codec_dai->id, 0xc2);
	wm8580_write(codec, 0xe + dai->codec_dai->id, 0xf3);
	wm8580_write(codec, 0xc + dai->codec_dai->id, 0x133);
	wm8580_write(codec, 0xd + dai->codec_dai->id, 0x123);
	wm8580_write(codec, 0xf + dai->codec_dai->id, 0x24);

	/*left->left&right*/
	//wm8580_write(codec, 0x10 + dai->codec_dai->id, 0x9);
	wm8580_write(codec, 0x10 + dai->codec_dai->id, 0x5);

	wm8580_write(codec, 0x11 + dai->codec_dai->id, 0x0);
	wm8580_write(codec, 0x12 + dai->codec_dai->id, 0x0);
	wm8580_write(codec, 0x13 + dai->codec_dai->id, 0x0);
	wm8580_write(codec, 0x1c + dai->codec_dai->id, 0x1cf);
...
static int wm8580_set_saif_dai_fmt(struct snd_soc_codec_dai *codec_dai,
				      unsigned int fmt)
{
#if 0
	struct snd_soc_codec *codec = codec_dai->codec;
...
struct snd_soc_codec_dai wm8580_dai[] = {
...
#endif
	{
		...
		.ops = {
			 .hw_params = wm8580_saif_hw_params,
		 },
		.dai_ops = {
			 .set_fmt = wm8580_set_saif_dai_fmt,
			 .set_clkdiv = wm8580_set_dai_clkdiv,
			 .set_pll = wm8580_set_dai_pll,
		 },
	},
