...
SND_SOC_DAPM_MICBIAS("Mic Bias", WM8753_PWR1, 5, 1),
...
static int wm8753_init(struct snd_soc_device *socdev)
{
	...
	wm8753_write(codec, WM8753_LOUT2V, reg | 0x014f);
	reg = wm8753_read_reg_cache(codec, WM8753_ROUT2V);
	wm8753_write(codec, WM8753_ROUT2V, reg | 0x014f);
	reg = wm8753_read_reg_cache(codec, WM8753_LINVOL);
	wm8753_write(codec, WM8753_LINVOL, reg | 0x0100);
	reg = wm8753_read_reg_cache(codec, WM8753_RINVOL);
	wm8753_write(codec, WM8753_RINVOL, reg | 0x0100);
	reg = wm8753_read_reg_cache(codec, WM8753_GPIO1);	// YX_MODI set GPIO4 to pull up
	wm8753_write(codec, WM8753_GPIO1, (reg & (~7)) | 3<<0);	//
	reg = wm8753_read_reg_cache(codec, WM8753_OUTCTL);	// YX_MODI headphone switch enable
	wm8753_write(codec, WM8753_OUTCTL, reg | 1<<2 | 3<<5);	//
	...
	wm8753_write(codec, WM8753_LINVOL, 0x13f);
	wm8753_write(codec, WM8753_MICBIAS, 0x100);
	wm8753_write(codec, WM8753_PWR1, wm8753_read_reg_cache(codec, WM8753_PWR1) | (1<<5));		//YX_MODI add for power on micbias
#endif
