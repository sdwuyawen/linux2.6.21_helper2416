...
s3c_irqext_type(unsigned int irq, unsigned int type)
{
	...
	void __iomem *gpup_reg;		// YX_MODI add 
	...
	if ((irq >= IRQ_EINT0) && (irq <= IRQ_EINT3))
	{
		...
		gpup_reg  = S3C2410_GPFUP;	// YX_MODI add
	...
	else if ((irq >= IRQ_EINT8) && (irq <= IRQ_EINT15))
	{
		...
		gpup_reg  = S3C2410_GPGUP;	// YX_MODI add
...
// YX_MODI add for pull up io port when use it as external interrupt begin
	value = __raw_readl(gpup_reg);
	value = (value & ~(3 << gpcon_offset)) | (0x02 << gpcon_offset);
	__raw_writel(value, gpup_reg);
// YX_MODI add for pull up io port when use it as external interrupt end
