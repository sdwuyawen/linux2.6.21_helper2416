...
{ 
	...
	if(!(ep->hcpriv))
	{
		...
		ep->hcpriv = (void *)(target_ed);
	} // if(!(ep->hcpriv))
	else
	{
		dev_addr = usb_pipedevice(urb->pipe);
		if(((ed_t *)(ep->hcpriv))->ed_desc.device_addr != dev_addr)
		{
			((ed_t *)ep->hcpriv)->ed_desc.device_addr = dev_addr;
		}
	}

	target_ed = (ed_t *)ep->hcpriv;
