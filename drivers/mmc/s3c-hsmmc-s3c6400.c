 ...
 *
 * $Id: s3c-hsmmc-s3c6400.c,v 1.1 2007/12/07 23:43:14 ihlee215 Exp $
 *
...
#if defined(CONFIG_HSMMC_PROC_DATA) || defined(CONFIG_PM)
...
static irqreturn_t s3c_hsmmc_irq (int irq, void *dev_id)
{
	...
	printk(PFX "got mask = 0x%08x\n", s3c_hsmmc_readl(S3C_HSMMC_NORINTSTSEN));
	printk(PFX "got signal = 0x%08x\n", s3c_hsmmc_readl(S3C_HSMMC_NORINTSIGEN));
#endif
...
static void s3c_hsmmc_request (struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (likely(s3c_hsmmc_readl(S3C_HSMMC_PRNSTS) & S3C_HSMMC_CARD_PRESENT)) {
...
static void s3c_hsmmc_set_clock (struct s3c_hsmmc_host *host, ulong clock)
{
	...
	if (clock > 25000000) {
		if (host->mmc->card_selected && mmc_card_movinand(host->mmc->card_selected))
	...
	if (clock == 0) {
		DBG(" In case of 0 Hz, DO nothing ..\n");
		return;
	}
	...
	if (host->mmc->card_selected && mmc_card_movinand(host->mmc->card_selected))
		s3c_hsmmc_writel(0x00004100 | (clk_src << 4), S3C_HSMMC_CONTROL2);
	else
		s3c_hsmmc_writel(0x00000100 | (clk_src << 4), S3C_HSMMC_CONTROL2);
#else
	s3c_hsmmc_writel(0x0000c100 | (clk_src << 4), S3C_HSMMC_CONTROL2);
#endif

	if (clock > 25000000) {
		if (host->mmc->card_selected && mmc_card_movinand(host->mmc->card_selected))
			s3c_hsmmc_writel(host->ctrl3[2], S3C_HSMMC_CONTROL3); // movinand
		else
			s3c_hsmmc_writel(host->ctrl3[1], S3C_HSMMC_CONTROL3);
	} else {
		if (host->mmc->card_selected && mmc_card_movinand(host->mmc->card_selected))
			s3c_hsmmc_writel(host->ctrl3[2], S3C_HSMMC_CONTROL3); // movinand
		else
			s3c_hsmmc_writel(host->ctrl3[0], S3C_HSMMC_CONTROL3);
	}
