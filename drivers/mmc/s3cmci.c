 ...
 *
 * $Id: s3cmci.c,v 1.1 2007/07/17 01:21:57 ihlee215 Exp $
 *
...
static irqreturn_t s3c_sdi_irq(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (sdi_csta & S3C2410_SDICMDSTAT_CMDTOUT) {
	...
	if (sdi_fsta & S3C2410_SDIFSTA_FIFOFAIL) {
...
#ifdef CONFIG_S3CMCI_DEBUG
		printk(PFX "timeout(%d) - DSTA: %08x\n", mrq->cmd->opcode, readl(host->base + S3C2410_SDIDSTA));
		{
			int i;
			printk("cmd_idx: %d\n", cmd_idx);
			for (i=0; i<40; i++) {
...
void s3c_sdi_dma_setup(struct s3c_sdi_host *host, eDMAPurpose_t purpose)
{
	...
	case DMAP_READ:
		dest_per = 0;
		src_per  = S3C2410_DMA3_SDMMC;
		flowctrl=S3C2410_DMA_PER2MEM;
		break;

	case DMAP_WRITE:
		dest_per= S3C2410_DMA3_SDMMC;
		src_per = 0;
		flowctrl=S3C2410_DMA_MEM2PER;
		break;
	...
	s3c_dma_devconfig(host->dma,host->subchannel,flowctrl,src_per,dest_per, host->mem->start + S3C2410_SDIDAT);
	s3c_dma_config(host->dma,host->subchannel, 4,4);
	s3c_dma_set_buffdone_fn(host->dma,host->subchannel, s3c_sdi_dma_done_callback);
	s3c_dma_setflags(host->dma,host->subchannel, S3C2410_DMAF_AUTOSTART);
}
#else
void s3c_sdi_dma_setup(struct s3c_sdi_host *host, eDMAPurpose_t purpose)
{
	...
	s3c_dma_devconfig(host->dma, source, 3, host->mem->start + S3C2410_SDIDAT);
	s3c_dma_config(host->dma, 4, 0, S3C2410_REQSEL_SDMMC | (1<<0));
	s3c_dma_set_buffdone_fn(host->dma, s3c_sdi_dma_done_callback);
	s3c_dma_setflags(host->dma, S3C2410_DMAF_AUTOSTART);
}
#endif
...
static void s3c_sdi_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	sdi_ccon = mrq->cmd->opcode & S3C2410_SDICCON_INDEX;
	sdi_ccon|= (S3C2410_SDICCON_SENDERHOST | S3C2410_SDICCON_CMDSTART);
	...
	sdi_imsk = (S3C2410_SDIIMSK_RESPONSEND | S3C2410_SDIIMSK_CRCSTATUS);
	...
	if (RSP_TYPE(mmc_resp_type(mrq->cmd))) {
		...
		sdi_ccon |= S3C2410_SDICCON_WAITRSP;
	...
	if (mrq->cmd->flags & MMC_RSP_136) {
		sdi_ccon|= S3C2410_SDICCON_LONGRSP;
	}
	...
	if (mrq->data) {
		...
		if (mrq->data->flags & MMC_DATA_WRITE) {
			...
			sdi_dcon |= S3C2410_SDIDCON_XFER_TX;
			sdi_dcon |= S3C2410_SDIDCON_DSTART;
			s3c_sdi_dma_setup(host, DMAP_WRITE);
			dma_dir = DMA_TO_DEVICE;
		} else {
			sdi_dcon |= S3C2410_SDIDCON_RXAFTERCMD;
			sdi_dcon |= S3C2410_SDIDCON_XFER_RX;
			sdi_dcon |= S3C2410_SDIDCON_DSTART;
			s3c_sdi_dma_setup(host, DMAP_READ);
			dma_dir = DMA_FROM_DEVICE;
		}

		sdi_fsta = S3C2410_SDIFSTA_FRST;
		__raw_writel(sdi_fsta,host->base + S3C2410_SDIFSTA);
	...
	writel(sdi_carg, host->base + S3C2410_SDICARG);
	writel(sdi_dcon, host->base + S3C2410_SDIDCON);
	/* This initiates transfer */
	writel(sdi_ccon, host->base + S3C2410_SDICCON);
	...
	/* Cleanup controller */
	writel(0, host->base + S3C2410_SDICARG);
	writel(0, host->base + S3C2410_SDIDCON);
	writel(0, host->base + S3C2410_SDICCON);
	writel(0, host->base + S3C2410_SDIIMSK);
	...
	if (mrq->data) {
		...
		if (mrq->data->error != MMC_ERR_NONE) {
#ifdef CONFIG_ARCH_MDIRAC3
			s3c_dma_ctrl(host->dma,host->subchannel, S3C2410_DMAOP_FLUSH);
#else
			s3c_dma_ctrl(host->dma, S3C2410_DMAOP_FLUSH);
#endif
...
static void s3c_sdi_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	writel(0, host->base + S3C2410_SDICON);
	udelay(1000);
	...
	case MMC_POWER_UP:
		...
		s3c_gpio_cfgpin(S3C2410_GPA8, S3C2410_GPA8_SDIO_CMD);
		s3c_gpio_cfgpin(S3C2410_GPA9, S3C2410_GPA9_SDIO_DAT0);
		s3c_gpio_cfgpin(S3C2410_GPA10, S3C2410_GPA10_SDIO_DAT1);
		s3c_gpio_cfgpin(S3C2410_GPA11, S3C2410_GPA11_SDIO_DAT2);
		s3c_gpio_cfgpin(S3C2410_GPA12, S3C2410_GPA12_SDIO_DAT3);
		s3c_gpio_cfgpin(S3C2410_GPA13, S3C2410_GPA13_SDIO_CLK);
		sdi_con = S3C2410_SDICON_CLKTYPE_MMC | (1 << 6);
#else
		s3c_gpio_cfgpin(S3C2410_GPE5, S3C2410_GPE5_SDCLK);
		s3c_gpio_cfgpin(S3C2410_GPE6, S3C2410_GPE6_SDCMD);
		s3c_gpio_cfgpin(S3C2410_GPE7, S3C2410_GPE7_SDDAT0);
		s3c_gpio_cfgpin(S3C2410_GPE8, S3C2410_GPE8_SDDAT1);
		s3c_gpio_cfgpin(S3C2410_GPE9, S3C2410_GPE9_SDDAT2);
		s3c_gpio_cfgpin(S3C2410_GPE10, S3C2410_GPE10_SDDAT3);
#endif

		if (mmc->mode == MMC_MODE_MMC) {
			sdi_con = S3C2410_SDICON_CLKTYPE_MMC;
		}
		break;

	case MMC_POWER_OFF:
	default:
		sdi_con = S3C2410_SDICON_RESET;
		writel(sdi_con, host->base + S3C2410_SDICON);
		while(readl(host->base + S3C2410_SDICON) & S3C2410_SDICON_RESET);
#ifdef CONFIG_ARCH_S3C2460
		s3c_gpio_cfgpin(S3C2410_GPA13, S3C2410_GPA13_OUTP);
#else
		s3c_gpio_cfgpin(S3C2410_GPE5, S3C2410_GPE5_OUTP);
#endif
		break;
	}
	...
	/* Set CLOCK_ENABLE */
	if (ios->clock)
		sdi_con |= S3C2410_SDICON_CLKENABLE;
	else
		sdi_con &=~S3C2410_SDICON_CLKENABLE;
...
	 */
...
#ifdef CONFIG_S3C2443_EVT1
	/* EXTINT0 S3C2443 EVT1 workaround */
	tmp = __raw_readl(S3C2410_EXTINT0);
	s3c_swap_4bit(tmp);
	__raw_writel(tmp | (1<<7), S3C2410_EXTINT0);
#endif
	s3c_gpio_cfgpin(S3C2410_GPF1, S3C2410_GPF1_EINT1);
#elif defined(CONFIG_ARCH_S3C2460)
	s3c_gpio_cfgpin(S3C2410_GPJ3, S3C2410_GPJ3_EXT_INT3);
#elif defined (CONFIG_MACH_SMDK2412)
	s3c_gpio_cfgpin(S3C2410_GPG10, S3C2410_GPG10_EINT18);
#endif
	...
	if (s3c_dma_request(S3C2410_SDI_DMA, &s3c_sdi_dma_client, NULL)) {
