 ...
 *
 * $Id: s3c-hsmmc-sanjose2.c,v 1.6 2008/04/11 05:52:14 jsgood Exp $
 *
 ...
 *
 *  Note1: s3c-hsmmc-rimini is for TomTom Rimini Evaluation Platform.
 *  Note2: On Rimini, 3 channels are used at same time and this driver supports that.
 *  Ch0: On board moviNAND
 *  Ch1: On board moviNAND
 *  Ch2: Card type microSD
 */
...
static irqreturn_t s3c_hsmmc_irq_cd (int irq, void *dev_id)
{
	...
	printk("s3c-hsmmc.2: card inserted or removed.\n");
...
static void s3c_hsmmc_set_clock (struct s3c_hsmmc_host *host, ulong clock)
{
	...
	/* CONTROL3 */
	if (clock > 25000000)
		s3c_hsmmc_writel(cfg->fd_ctrl[cardtype].ctrl3[1], S3C_HSMMC_CONTROL3);
	else
		s3c_hsmmc_writel(cfg->fd_ctrl[cardtype].ctrl3[0], S3C_HSMMC_CONTROL3);
...
static int s3c_hsmmc_probe (struct platform_device *pdev)
{
	...
	if (plat_data->hwport == 2) {
		...
		s3c_hsmmc_irq_cd(host->irq_cd, host);
	}
