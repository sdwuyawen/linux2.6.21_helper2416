...
#include "pwm-s3c6400.h"


s3c6400_pwm_chan_t s3c_chans[S3C_PWM_CHANNELS];


static inline void
s3c6400_pwm_buffdone(s3c6400_pwm_chan_t *chan, void *dev)
{
...
static int s3c6400_pwm_start (int channel)
{
	unsigned long tcon;
...
static irqreturn_t s3c6400_pwm_irq(int irq, void *devpw)
{
	s3c6400_pwm_chan_t *chan = (s3c6400_pwm_chan_t *)devpw;
	void *dev=chan->dev;

	/* modify the channel state */
	s3c6400_pwm_buffdone(chan, dev);

	return IRQ_HANDLED;
}


int s3c6400_pwm_request(pwmch_t  channel, s3c_pwm_client_t *client, void *dev)
{
	s3c6400_pwm_chan_t *chan = &s3c_chans[channel];
	unsigned long flags;
	int err;
	...
	if (!chan->irq_claimed) {
		...
		err = request_irq(chan->irq, s3c6400_pwm_irq, SA_INTERRUPT,
...
int s3c6400_pwm_free (pwmch_t channel, s3c_pwm_client_t *client)
{
	s3c6400_pwm_chan_t *chan = &s3c_chans[channel];
	unsigned long flags;
...
int s3c6400_pwm_set_buffdone_fn(pwmch_t channel, s3c_pwm_cbfn_t rtn)
{
	s3c6400_pwm_chan_t *chan = &s3c_chans[channel];
...
#define s3c6400_pwm_suspend NULL
#define s3c6400_pwm_resume  NULL

struct sysdev_class pwm_sysclass = {
	set_kset_name("s3c-pwm"),
	.suspend	= s3c6400_pwm_suspend,
	.resume		= s3c6400_pwm_resume,
};
