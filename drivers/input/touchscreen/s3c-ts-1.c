...
static irqreturn_t stylus_updown(int irqno, void *param)
{
	...
	//updown = (!(data0 & S3C2410_ADCDAT0_UPDOWN)) && (!(data1 & S3C2410_ADCDAT1_UPDOWN));dxl fix
...
static int __init s3c_ts_probe(struct platform_device *pdev)
{
	...
	if ((s3c_ts_cfg.delay&0xffff) > 0)
		//writel(s3c_ts_cfg.delay & 0xffff, base_addr+S3C2410_ADCDLY);dxl fix
	...
	ts->dev->evbit[0] =  BIT(EV_SYN) | BIT(EV_KEY) | BIT(EV_ABS);//dxl fix
